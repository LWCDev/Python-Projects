...
def city_search(city_section, state_nums, state_links, statelink, url):
    city_nums = city_section.find_all('span', class_="number-countries")
    city_links = city_section.find_all('a', href=True)
    print("First city!", city_links[0])
    if len(state_nums) != len(state_links):
        print("This method will NOT work")
    for idx, citlink in enumerate(city_links):
        citylink = 'https://www.dnb.com' + citlink['href']
        city_num = city_nums[idx].text
        city_num = city_num.strip().replace("(", "").replace(")", "").replace("\n", "").replace("'", "").replace(",",
                                                                                                                 "")
        city_num = int(city_num)
        if city_num >= 1000:
            print("More than 1000 results")
            max_pages = range(1, 21, 1)
        elif city_num < 1000:
            print("Less than 1000 results")
            max_pages = range(1, math.ceil(city_num / 50) + 1, 1)
        else:
            print("Couldn't find your results")
            max_pages = range(1, 3, 1)
        print(max_pages)
        citylink = citylink[:len(citylink) - 1]
        outlist = []
        for x in max_pages:
            print(x)
            saveurl = citylink + str(x)
            print(saveurl)
            savedata = (saveurl, statelink, url, 0, 0, 0)
            outlist.append(savedata)
        columns = ["url", "state_url", "country_url", "in_progress",
                   "complete", "failed"]
        save_df = pd.DataFrame(data=outlist, columns=columns)
        MongoUtilities.pandas_to_mongo(save_df, db=db2,
                                       collection="page_urls_test",
                                       keys=['url'])


def state_search(state_links, scraper, proxy, state_nums, url):
    for index, link in enumerate(state_links):
        # print(link)
        statelink = 'https://www.dnb.com' + link['href']
        print(statelink)
        try:
            page = scraper.get(statelink, headers=headers, timeout=30, proxies=proxy)
        except:
            print("Failure in fetching page on: ", statelink, " moving to next in the list")
            continue
        try:
            new_url = get_auth_url(page)
            ...
            print("Failure in get_auth function: ", statelink, "skipping to next in list")
            continue
        try:
            print("Trying to get cities")
            page = scraper.get(new_url, headers=headers, timeout=30, proxies=proxy)
            print("Got past page")
            soup = BeautifulSoup(page.text, 'lxml')
        except:
            print("Failure in second page fetch: ", statelink, "skipping to next in list")
            continue
        try:
            print("Now in section2.find_all")
            section2 = soup.find_all('div', class_='title')
            city_section = soup.find('div', {"id": "locationResults"})
            for divider in section2:
                if "Town" in divider.text:
                    city_search(city_section, state_nums, state_links, statelink, url)
        except:
            print("Failure while running city search, moving to next state")
            continue
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    state_section = soup.find('div', { "id" : "locationResults"} )
                    #print(state_section)
                    ...
                    for div in section:
                        ...
                        if 'State/Province' in div.text:
                            state_nums = state_section.find_all('span', class_="number-countries")
                            state_links = state_section.find_all('a', href=True)
                            #print(state_links)
                            # if len(state_nums) != len(state_links):
                            #     print("This method will NOT work")
                            state_search(state_links, scraper, proxy, state_nums, url)
...
if __name__ == "__main__":
    database_name = "dnb_search_v2_test"
    ...
    if returns['locked']:
        ...
        #     quit()
        #print(failist, '\n', len(failist))
        # failist= ["https://www.dnb.com/business-directory/company-information.psychiatric_and_substance_abuse_hospitals.us.html?page=1"]
        failist=["https://www.dnb.com/business-directory/company-information.health_care_and_social_assistance.us.html?page=1"]
